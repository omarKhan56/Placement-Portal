...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      !/^[A-Z][a-zA-Z ]*$/.test(formData.fullName)
    ) {
      setError("Full Name must start with a capital letter");
      setLoading(false);
      return;
    }

    if (formData.role === "student" && !/^\d{6}$/.test(formData.studentId)) {
    ...
    if (
      ...
      !formData.email.endsWith("@mgmplacement.org")
    ) {
      setError("Placement email must end with @mgmplacement.org");
      setLoading(false);
      return;
    }

    if (formData.role === "mentor" && !formData.email.endsWith("@mentor.org")) {
      setError("Mentor email must end with @mentor.org");
      setLoading(false);
      return;
    }
    ...
    if (
      ...
      !formData.email.endsWith("@employer.org")
    ) {
      setError("Employer email must end with @employer.org");
      setLoading(false);
      return;
    }
